...
const getDataMinMaxUtility = async (req, res) => {
    try {
        const {
            lotno,
            line,
        ...
        query = query.replace(':start_time', `'${start_time}'`).replace(':end_time', `'${end_time}'`).replace(':lotno', `'${lotno}'`);
        ...
        const result = {};
        for (const key in dataMinMax[0]) {
            if (dataMinMax[0].hasOwnProperty(key)) {
              const value = dataMinMax[0][key];
...
const getDataTrendUtility = async (req, res) => {
    try {
        const {
            lotno,
            line,
        ...
        // Create Query   
        let query = datasource[0].query;
        query = query.replace(':start_time', `'${start_time}'`).replace(':end_time', `'${end_time}'`);
